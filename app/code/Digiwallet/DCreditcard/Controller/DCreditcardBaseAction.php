<?php
namespace Digiwallet\DCreditcard\Controller;
 ...
 * @method POST
 */
class DCreditcardBaseAction extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    public function checkDigiwalletResult()
    {
        ...
        $testMode = false;//(bool) $this->scopeConfig->getValue('payment/dcreditcard/testmode', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $digiCore = new DigiwalletCore(
            $this->dcreditcard->getMethodType(),
            $this->scopeConfig->getValue('payment/dcreditcard/rtlo', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
            $language,
            $testMode
        );
        ...
        if ($paymentStatus) {
            $sql = "UPDATE ".$tableName." 
                ...
                AND method='" . $this->dcreditcard->getMethodType() . "'
                AND `digi_txid` = '" . $txId . "'";
            $db->query($sql);
            ...
            $transport = $this->transportBuilder
                ->setTemplateIdentifier(
                    $this->scopeConfig->getValue('payment/dcreditcard/email_template/failure', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
                    $storeScope
                )
                ->setTemplateOptions([

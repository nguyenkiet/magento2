<?php
namespace Digiwallet\DPaysafecard\Controller;

use Digiwallet\DCore\DigiwalletCore;

/**
 * Digiwallet DPaysafecard Report Controller
 *
 * @method POST
 */
class DPaysafeBaseAction extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    /**
     * When a customer return to website from Digiwallet DPaysafecard gateway after a payment is marked as successful.
    ...
    public function checkDigiwalletResult()
    {
        ...
        $testMode = false;//(bool) $this->scopeConfig->getValue('payment/dpaysafecard/testmode', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $digiCore = new DigiwalletCore(
            $this->dpaysafecard->getMethodType(),
            $this->scopeConfig->getValue('payment/dpaysafecard/rtlo', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
            $language,
            $testMode
        );
        ...
        if ($paymentStatus) {
            $sql = "UPDATE ".$tableName." 
                ...
                AND method='" . $this->dpaysafecard->getMethodType() . "'
                AND `digi_txid` = '" . $txId . "'";
            $db->query($sql);
            ...
            $transport = $this->transportBuilder
                ->setTemplateIdentifier(
                    $this->scopeConfig->getValue('payment/dpaysafecard/email_template/failure', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
                    $storeScope
                )
                ->setTemplateOptions([

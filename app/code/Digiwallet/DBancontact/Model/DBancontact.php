...
class DBancontact extends \Magento\Payment\Model\Method\AbstractMethod
{
    const METHOD_CODE = 'dbancontact';
    ...
    public function setupPayment($bankId = false)
    {
        ...
        if ($order->getGrandTotal() < $this->minAmount) {
            throw new \Magento\Checkout\Exception(
                __('Het totaalbedrag is lager dan het minimum van ' . $this->minAmount . ' euro voor ' . DBancontact::METHOD_CODE)
            );
        }
        
        if ($order->getGrandTotal() > $this->maxAmount) {
            throw new \Magento\Checkout\Exception(
                __('Het totaalbedrag is hoger dan het maximum van ' . $this->maxAmount . ' euro voor ' . DBancontact::METHOD_CODE)
            );
        }

        $orderId = $order->getRealOrderId();
        $language = ($this->localeResolver->getLocale() == 'nl_NL') ? "nl" : "en";
        $testMode = false;//(bool) $this->_scopeConfig->getValue('payment/dbancontact/testmode', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        
        $digiCore = new DigiwalletCore(
            $this->tpMethod,
            $this->_scopeConfig->getValue('payment/dbancontact/rtlo', \Magento\Store\Model\ScopeInterface::SCOPE_STORE),
            $language,
            $testMode
        );
